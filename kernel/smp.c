...
#define MP_ENTRY_BUS_SIZE           8u
#define MP_ENTRY_IOAPIC_SIZE        8u
#define MP_ENTRY_IOINT_SIZE         8u
#define MP_ENTRY_LOCALINT_SIZE      8u
...
// Keep this out-of-line so GCC does not fold the absolute low-memory BIOS read
// into an optimized expression that then triggers bogus array-bounds warnings.
static __attribute__((noinline)) uint16_t smp_read_bios_word(uint32_t addr) {
    uint16_t value;
    __asm__ volatile("movw %1, %0"
                     : "=r"(value)
                     : "m"(*(const volatile uint16_t *)(uintptr_t)addr));
    return value;
}

static const mp_floating_ptr_t *smp_find_mp_floating(const uint8_t *base, uint32_t length) {
    for (uint32_t off = 0; off + sizeof(mp_floating_ptr_t) <= length; off += 16) {
...
static const mp_floating_ptr_t *smp_locate_mp_floating(void) {
    ...
    if (ebda_segment != 0) {
        mp = smp_find_mp_floating((const uint8_t *)((uint32_t)ebda_segment << 4), 1024);
        if (mp) return mp;
    }

    if (base_mem_kb >= 1) {
        uint32_t top_of_base_mem = (uint32_t)base_mem_kb * 1024u;
        mp = smp_find_mp_floating((const uint8_t *)(top_of_base_mem - 1024u), 1024);
        if (mp) return mp;
    }
...
static uint32_t smp_count_mp_cpus(const mp_config_table_t *config) {
    ...
    while (entry < end) {
        switch (entry[0]) {
            ...
            case 1:
                entry += MP_ENTRY_BUS_SIZE;
                break;
            case 2:
                entry += MP_ENTRY_IOAPIC_SIZE;
                break;
            case 3:
                entry += MP_ENTRY_IOINT_SIZE;
                break;
            case 4:
                entry += MP_ENTRY_LOCALINT_SIZE;
                break;
            default:
                return count;
        }
